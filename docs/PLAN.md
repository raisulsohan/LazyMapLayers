...
| 6 — Labels | IN PROGRESS 2026-09-22: one label template per map - colour, size, halo, capitals, dots and a font picked up from a text layer the user styled, with every field falling back to the look (D36, LB2). Left: keep-out zones, a timeline-wide auto-label engine, callouts from the template |
