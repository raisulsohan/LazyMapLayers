...
| 4 — Data | IN PROGRESS 2026-09-22: any named OpenStreetMap feature for the area in the preview, through Overpass - nine kinds, relations assembled into polygons with their holes, arriving as an ordinary import, polite and cached on disk, with the credit the licence asks for (D38, OSM1). Already in place from earlier phases: offline Natural Earth, districts from geoBoundaries (D27), region and elevation downloads (D28), imagery packs (D29), offline search, and imports of GPX, KML, KMZ, GeoJSON, CSV and shapefiles (D22, D25). Left: reverse geocoding, Google Earth Studio tracking import, Sentinel-2 and orthophoto imagery, a feature browser with merge, dissolve, buffer and CSV join |
