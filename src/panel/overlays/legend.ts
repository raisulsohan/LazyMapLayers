...
import { legendLayout, legendPosition, type LegendCorner } from "../../core/style/legend.ts";
...
export async function addLegend(mapId: string, fill: DataFill, options: LegendOptions = {}): Promise<LegendResult> {
  ...
  const layout = legendLayout(rows, { height: info.height, title, titleWidth: measure(title, script, titleSize, 600, 0) });
  ...
  return callHost<LegendResult>("addLegend", {
    ...
    fonts,
    rows: layout.rows.map((row) => ({ color: hexToRgb(row.color), label: row.label, size: row.size, swatch: row.swatch, text: row.text, radius: Math.round(2 * layout.scale) }))
  });
}
